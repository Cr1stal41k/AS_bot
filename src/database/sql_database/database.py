...
class SQLDatabase:
    ...
    def find_all_users(self) -> List[int]:
        return self._get_users_by_bot_name(self.bot_name)

    def add_user(self, telegram_id: int) -> bool:
        self._add_user(telegram_id,self.bot_name)
        return True
    ...
    @connect_db
    def _add_user(self,telegram_id,bot_name,db):
        db_model = UserModel(telegram_id=telegram_id,bot_name=bot_name)
        try:
            db.add(db_model)
            db.commit()
    ...
    @connect_db
    def _get_users_by_bot_name(self,bot_name,db):
        return db.query(UserModel).filter(UserModel.bot_name==bot_name).all()

    @connect_db
    def _get_user_by_telegram_id(self,telegram_id,db):
        return db.query(UserModel).filter(UserModel.bot_name==bot_name,UserModel.telegram_id==telegram_id).one()
    
    @connect_db
    def _delete_user_by_telegram_id(self,telegram_id,db):
        db.query(UserModel).filter(UserModel.bot_name==bot_name,UserModel.telegram_id==telegram_id).delete()